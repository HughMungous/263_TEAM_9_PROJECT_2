...
def eliminatePoorRoutes(routes, demands, minLenToKeep: int = 2, maxDuration: float = 6.0):
    ...
    durations = [calculateDuration(route, demands) for route in routes]

    newRoutes = [routes[i] for i in range(len(durations)) if durations[i] < maxDuration]
    return newRoutes

def findInitalSolution(day: str, demands, locations):
    ...
    for region in ["North", "West", "Central","South"]:
        ...
        routes = getRoutes(regionalDemands)
        routes = eliminatePoorRoutes(routes, regionalDemands)
        stores = regionalDemands.keys()
        durations = [calculateDuration(route, regionalDemands) for route in routes]
...
def runSimulationInstance(demands: pd.DataFrame, routes: List[List[str]], trafficMultiplier: float, trafficStd: float = 0.1, simulationNumber: int = 1000):
    ...
    return {
        ...
        "durations (average)": routeDurations, 
        "costs": routeCosts}
...
def simulateUncertainty(initialSolutions: Dict[str, List[str]], trafficMultipliers: Dict[str, Tuple[str]], days: List[str] = ["WeekdayAvg", "Saturday"], trafficStd=0.1):
    ...
    demands = {
        "WeekdayAvg": dataInput.readDataWithStats(), 
        "Saturday": dataInput.readSaturdayWithStats()
    }
    ...
    for day in days:
        ...
        dayResults = {}
        # Simulating the morning period
        dayResults["morning"] = runSimulationInstance(demands=dayDemands, routes=initialSolutions[day], trafficMultiplier=trafficMultipliers[day][0], trafficStd=trafficStd)
        dayResults["evening"] = runSimulationInstance(demands=dayDemands, routes=initialSolutions[day], trafficMultiplier=trafficMultipliers[day][1], trafficStd=trafficStd)
        
        simulationResults[day] = dayResults
    return simulationResults

def simulateStoreClosures():
...
def main():
    np.random.seed(508)
    # this should be moved to a json:
    trafficMultipliers = {
        "WeekdayAvg": (1.399,1.564),
        "Saturday": (1.208,1.204)
    }
    ...
    simulationResults = simulateUncertainty(initialSolutions=initialResults, trafficMultipliers=trafficMultipliers, trafficStd=0.1)
